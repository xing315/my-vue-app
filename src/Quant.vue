...
import { loadResearchState, migrateLocalHoldings, ensureDefaultAlertRules, toggleWatchlist, addCloudHolding, removeCloudHolding, markAlertRead } from './research.js'
...
export default {
  props: { user: Object },
  data: () => ({ data:null, loading:true, query:'', industry:'all', minScore:0, selected:null, stockDetail:null, detailLoading:false, detailError:'', chartWindow:120, budget:100000, holdings:[], watchlist:[], alerts:[], researchError:'', watchRisk:'all', form:{code:'',name:'',cost:'',shares:''}, tab:'cockpit', error:'', demo:false, coverageCount:0, brief:'',briefLoading:false,briefError:'',quotaRemaining:null }),
  computed: {
    ...
    watchStocks(){return this.watchlist.map(code=>this.stocks.find(s=>s.code===code)).filter(Boolean).filter(s=>this.watchRisk==='all'||(this.watchRisk==='risk'?(s.score<55||s.flags?.length):s.score>=70))},
    unreadAlerts(){return this.alerts.filter(a=>!a.readAt)},
    timeline(){const events=this.alerts.map(a=>({type:'signal',date:a.trade_date,title:a.title,detail:a.reason,severity:a.severity,symbol:a.symbol,source:a.source,signalId:a.signalId,readAt:a.readAt}));const ranks=this.dailyRecommendations.filter(r=>r.rankChange||r.previousRank==null).slice(0,8).map(r=>({type:'rank',date:this.data?.updatedAt?.slice(0,10),title:r.previousRank?'Top 30 排名变化':'新进入重点研究',detail:`${r.name} · 当前第 ${r.rank} 名`,severity:'info',symbol:r.code,source:'盘后规则排名'}));return [...events,...ranks].sort((a,b)=>String(b.date).localeCompare(String(a.date))).slice(0,20)},
  ...
  async mounted() { await this.load(); await this.loadPersonal();const pending=sessionStorage.getItem('quant-open-symbol');if(pending){sessionStorage.removeItem('quant-open-symbol');const stock=this.stocks.find(s=>s.code===pending);if(stock)this.select(stock)} },
  watch:{user(){this.loadPersonal()}},
  methods: {
    ...
    async loadPersonal(){this.researchError='';try{const state=await loadResearchState(this.user);this.watchlist=state.watchlist;this.holdings=state.holdings;this.alerts=state.alerts;if(this.user){try{await ensureDefaultAlertRules(this.user)}catch(e){if(e.message?.includes('schema cache')||e.code==='PGRST205')this.researchError='新驾驶舱数据库尚未部署，现有自选和持仓仍可使用';else throw e}await migrateLocalHoldings(this.user)}}catch(e){this.researchError=`个人投研数据读取失败：${e.message}`}},
    async toggleWatch(s){try{const active=this.watchlist.includes(s.code);await toggleWatchlist(this.user,s.code,active);this.watchlist=active?this.watchlist.filter(c=>c!==s.code):[...this.watchlist,s.code]}catch(e){this.researchError=e.message}},
    isWatched(code){return this.watchlist.includes(code)},
    async readAlert(item){if(!this.user||!item.signalId||item.readAt)return;try{await markAlertRead(this.user,item.signalId);const found=this.alerts.find(a=>a.signalId===item.signalId);if(found)found.readAt=new Date().toISOString()}catch(e){this.researchError=e.message}},
    openTimeline(item){this.readAlert(item);const stock=this.stocks.find(s=>s.code===item.symbol);if(stock)this.select(stock)},
    async generateBrief(stock=this.selectedStock){if(!this.user){this.briefError='请先登录后生成研究简报';return}if(!stock)return;this.brief='';this.briefError='';this.briefLoading=true;try{const {data,error}=await supabase.functions.invoke('deepseek-chat',{body:{mode:'research_brief',symbol:stock.code,selectedNewsIds:[]}});if(error)throw error;if(!data?.answer)throw new Error('AI 未返回有效简报');this.brief=data.answer;this.quotaRemaining=data.remaining}catch(e){this.briefError=e.message?.includes('non-2xx')?'研究简报服务未配置、额度不足或数据库迁移尚未执行':e.message}finally{this.briefLoading=false}},
    ...
    async addHolding() { const h={code:this.form.code.trim(),name:this.form.name.trim()||this.form.code.trim(),cost:Number(this.form.cost),shares:Number(this.form.shares)}; if(!/^[036]\d{5}$/.test(h.code)||!h.cost||!h.shares||h.shares%100){this.researchError='请输入有效 A 股代码、成本和 100 股整数倍数量';return}try{if(this.user)this.holdings.push(await addCloudHolding(this.user,h));else{this.holdings.push(h);this.save()}this.form={code:'',name:'',cost:'',shares:''}}catch(e){this.researchError=e.message} },
    async removeHolding(i) { const h=this.holdings[i];try{if(this.user&&h.id)await removeCloudHolding(this.user,h.id);this.holdings.splice(i,1);if(!this.user)this.save()}catch(e){this.researchError=e.message} },
...
<template>
  <div class="quant-page" v-if="!loading">
    ...
    <div v-if="researchError" class="demo-banner personal-error"><b>提示</b><span>{{ researchError }}</span><button @click="researchError=''">关闭</button></div>
    <header class="q-hero">
    ...
    <nav class="q-tabs"><button v-for="t in [{id:'cockpit',name:`投研驾驶舱${unreadAlerts.length?` · ${unreadAlerts.length}`:''}`},{id:'watchlist',name:'我的自选'},{id:'top30',name:'今日前30'},{id:'candidates',name:'市场与候选'},{id:'detail',name:'个股分析'},{id:'portfolio',name:'我的持仓'},{id:'validation',name:'模型与数据'}]" :key="t.id" :class="{active:tab===t.id}" @click="tab=t.id">{{ t.name }}</button></nav>

    <template v-if="tab==='cockpit'">
      <section class="cockpit-hero"><div><span>PERSONAL RESEARCH COCKPIT</span><h2>今天需要关注什么？</h2><p>{{ user?'自选、持仓和盘后信号已连接云端。':'公开市场数据可浏览；登录后启用自选、预警和 AI 简报。' }}</p></div><button :disabled="!user||briefLoading" @click="generateBrief(watchStocks[0]||selectedStock)">{{briefLoading?'生成中…':'生成首要标的简报'}}</button></section>
      <section class="cockpit-metrics"><article><span>市场上涨宽度</span><b>{{data.market.breadth}}%</b><small>风险状态 · {{data.market.risk}}</small></article><article><span>自选股票</span><b>{{watchlist.length}}</b><small>{{watchStocks.filter(s=>s.score>=70).length}} 只评分 ≥70</small></article><article><span>待处理预警</span><b>{{unreadAlerts.length}}</b><small>仅展示盘后可解释信号</small></article><article><span>模型日期</span><b>{{data.updatedAt.slice(5,10)}}</b><small>{{data.modelVersion}}</small></article></section>
      <section class="cockpit-grid"><article class="panel-q"><span>TODAY TIMELINE</span><h3>今日发生了什么</h3><div v-if="timeline.length" class="signal-list"><button v-for="item in timeline" :key="`${item.type}-${item.symbol}-${item.title}`" :class="[item.severity,{read:item.readAt}]" @click="openTimeline(item)"><i></i><span><b>{{item.title}}</b><small>{{item.detail}}</small><em>{{item.date}} · {{item.source}}</em></span></button></div><div v-else class="detail-state">{{user?'暂无新的盘后信号':'登录后查看与你的自选和持仓相关的信号'}}</div></article>
        <article class="panel-q"><span>WATCHLIST PULSE</span><h3>自选股脉搏</h3><div v-if="watchStocks.length" class="watch-pulse"><button v-for="s in watchStocks.slice(0,8)" :key="s.code" @click="select(s)"><span><b>{{s.name}}</b><small>{{s.code}} · {{s.industry}}</small></span><strong :class="tone(s.score)">{{s.score}}</strong><em :class="s.change>=0?'up':'down'">{{s.change>=0?'+':''}}{{s.change}}%</em></button></div><div v-else class="detail-state">从候选列表或个股详情加入自选，驾驶舱会聚合变化。</div></article></section>
      <section v-if="brief||briefError" class="panel-q brief-card"><span>AI RESEARCH BRIEF</span><h3>证据约束研究简报</h3><p v-if="brief" class="brief-content">{{brief}}</p><p v-if="briefError" class="brief-error">{{briefError}}</p><footer v-if="quotaRemaining!==null">今日剩余 {{quotaRemaining}} / 10 次 · AI 只解释，不参与评分</footer></section>
    </template>

    <template v-else-if="tab==='watchlist'">
      <section class="section-head"><div><span>CLOUD WATCHLIST</span><h2>我的自选</h2></div><p>{{user?'跨设备同步 · 可直接筛选':'请登录后使用云端自选股'}}</p></section>
      <div class="filters"><input v-model="query" placeholder="搜索代码或名称"><select v-model="industry"><option value="all">全部行业</option><option v-for="i in industries" :key="i">{{i}}</option></select><select v-model="watchRisk"><option value="all">全部信号</option><option value="strong">评分 ≥70</option><option value="risk">风险关注</option></select></div>
      <section class="stock-table"><div class="s-row s-head"><span>标的</span><span>评级</span><span>最新价</span><span>核心证据</span><span>风险标签</span><span></span></div><div class="s-row" v-for="s in watchStocks.filter(s=>(industry==='all'||s.industry===industry)&&(!query||`${s.code}${s.name}`.includes(query)))" :key="s.code"><span><b>{{s.name}}</b><small>{{s.code}} · {{s.industry}}</small></span><span><i class="score" :class="tone(s.score)">{{s.score}}</i><small>置信 {{s.confidence}}%</small></span><span><b>¥{{s.price.toFixed(2)}}</b><small :class="s.change>=0?'up':'down'">{{s.change>=0?'+':''}}{{s.change}}%</small></span><span><b>{{s.reason}}</b></span><span><small class="flags">{{s.flags?.join(' · ')||'暂无'}}</small></span><span><button @click="select(s)">分析 →</button><button @click="toggleWatch(s)">移除</button></span></div><div v-if="!watchStocks.length" class="empty-q">还没有自选股票</div></section>
    </template>

    <template v-else-if="tab==='top30'">
      ...
      <section v-if="dailyRecommendations.length" class="recommendation-grid">
        <article v-for="item in dailyRecommendations" :key="item.code" class="recommendation-card">
          ...
          <footer><span>置信 {{ item.confidence }}%</span><span>建议仓位 {{ item.position?.[0] }}–{{ item.position?.[1] }}%</span><button @click="toggleWatch(stocks.find(s=>s.code===item.code)||item)">{{isWatched(item.code)?'★ 已自选':'☆ 加自选'}}</button><button @click="select(stocks.find(s=>s.code===item.code)||item)">完整分析 →</button></footer>
        </article>
      </section>
    ...
    <template v-else-if="tab==='candidates'">
      ...
      <section class="stock-table">
        ...
        <div class="s-row" v-for="s in filtered" :key="s.code">
          <span><b>{{ s.name }}</b><small>{{ s.code }} · {{ s.industry }}</small></span><span><i class="score" :class="tone(s.score)">{{ s.score }}</i><small>{{ rating(s.score) }} · 置信 {{ s.confidence }}%</small></span><span><b>¥{{ s.price.toFixed(2) }}</b><small :class="s.change>=0?'up':'down'">{{ s.change>=0?'+':'' }}{{ s.change }}%</small></span><span><b>{{ s.reason }}</b><small v-if="s.flags.length" class="flags">{{ s.flags.join(' · ') }}</small></span><span><b>{{ s.position[1] ? `${s.position[0]}–${s.position[1]}%` : '不建议' }}</b><small>单股上限 8%</small></span><span><button @click="toggleWatch(s)">{{isWatched(s.code)?'★':'☆'}}</button><button @click="select(s)">分析 →</button></span>
        </div><div v-if="!filtered.length" class="empty-q">当前条件下没有候选股票</div>
      </section>
    </template>

    <template v-else-if="tab==='detail' && selectedStock">
      <section class="detail-hero"><div><span>{{ selectedStock.code }} · {{ selectedStock.industry }}</span><h2>{{ selectedStock.name }}</h2><p>{{ rating(selectedStock.score) }} · 置信度 {{ selectedStock.confidence }}%</p><button class="hero-action" @click="toggleWatch(selectedStock)">{{isWatched(selectedStock.code)?'★ 已加入自选':'☆ 加入自选'}}</button><button class="hero-action" :disabled="briefLoading" @click="generateBrief(selectedStock)">{{briefLoading?'生成中…':'AI 研究简报'}}</button></div><div class="hero-price"><b>¥{{ selectedStock.price.toFixed(2) }}</b><em :class="selectedStock.change>=0?'up':'down'">{{ selectedStock.change>=0?'+':'' }}{{ selectedStock.change }}%</em></div><div class="big-score" :class="tone(selectedStock.score)"><b>{{ selectedStock.score }}</b><span>/ 100</span></div></section>
      <section v-if="brief||briefError" class="panel-q brief-card"><span>AI RESEARCH BRIEF</span><h3>{{selectedStock.name}} · 证据约束简报</h3><p v-if="brief" class="brief-content">{{brief}}</p><p v-if="briefError" class="brief-error">{{briefError}}</p><footer v-if="quotaRemaining!==null">今日剩余 {{quotaRemaining}} / 10 次</footer></section>
...
.personal-error{margin-top:8px;background:#ffe9e4;border-color:#e3a79b}.cockpit-hero{display:flex;align-items:center;justify-content:space-between;background:var(--green);color:white;border-radius:22px;padding:30px;margin-bottom:14px}.cockpit-hero span{font:700 10px monospace;letter-spacing:1.5px;color:var(--lime)}.cockpit-hero h2{font:37px Georgia,"Songti SC";margin:8px 0}.cockpit-hero p{margin:0;color:#bdd0ca}.cockpit-hero button{border:0;background:var(--lime);color:var(--green);padding:12px 17px;border-radius:999px;font-weight:700}.cockpit-metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.cockpit-metrics article{background:var(--paper);border:1px solid var(--line);border-radius:16px;padding:19px}.cockpit-metrics span,.cockpit-metrics small{display:block;color:var(--muted);font-size:10px}.cockpit-metrics b{display:block;font:28px Georgia;margin:12px 0 5px}.cockpit-grid{display:grid;grid-template-columns:1.2fr .8fr;gap:14px;margin-top:14px}.signal-list button,.watch-pulse button{width:100%;border:0;border-top:1px solid var(--line);background:none;text-align:left;padding:13px 4px;display:flex;align-items:center;gap:11px;cursor:pointer}.signal-list button>i{width:9px;height:9px;border-radius:50%;background:#5a8cff}.signal-list button.risk>i{background:#d65748}.signal-list button.attention>i{background:#e4a52e}.signal-list button.read{opacity:.55}.signal-list span{flex:1}.signal-list b,.signal-list small,.signal-list em,.watch-pulse b,.watch-pulse small{display:block}.signal-list small{margin:4px 0;font-size:10px;color:var(--muted)}.signal-list em{font-size:9px;color:#909994;font-style:normal}.watch-pulse button span{margin-right:auto}.watch-pulse strong{display:grid;place-content:center;width:38px;height:31px;border-radius:8px}.watch-pulse em{font-size:10px;font-style:normal}.hero-action{margin:13px 8px 0 0;border:1px solid #68877f;background:transparent;color:white;border-radius:999px;padding:8px 12px;font-size:10px}.brief-card{margin:14px 0}.brief-content{white-space:pre-wrap;line-height:1.8;font-size:12px}.brief-error{color:#a94335}.brief-card footer{font-size:9px;color:var(--muted)}
...
@media(max-width:900px){.quant-page{padding:22px 16px 60px}.q-hero,.cockpit-hero{align-items:start;flex-direction:column}.cockpit-hero button{margin-top:18px}.as-of{width:100%}.q-tabs{overflow-x:auto}.q-tabs button{white-space:nowrap}.market-grid,.cockpit-metrics{grid-template-columns:1fr 1fr}.cockpit-grid{grid-template-columns:1fr}.section-head{align-items:start;flex-direction:column}.filters{flex-wrap:wrap}.filters>input{min-width:0;flex:1}.filters label{width:100%;margin:0}.stock-table{overflow-x:auto}.s-row{min-width:980px}.detail-grid,.lower,.financial-grid,.recommendation-grid{grid-template-columns:1fr}.profile-strip{grid-template-columns:1fr 1fr}.holding-form{grid-template-columns:1fr 1fr}.holding-form button{min-height:42px}.validation-grid{grid-template-columns:1fr 1fr}.source-grid{grid-template-columns:1fr}.stock-chart{height:300px}.chart-summary{flex-wrap:wrap}}
@media(max-width:520px){.market-grid,.cockpit-metrics,.portfolio-summary,.validation-grid,.profile-strip{grid-template-columns:1fr}.q-hero h1{font-size:44px}.detail-hero{align-items:flex-start;flex-wrap:wrap}.big-score{width:80px}.budget-results,.valuation>div,.metric-grid{grid-template-columns:1fr}.holding-form{grid-template-columns:1fr}.chart-head{display:block}.range-buttons{margin-top:12px}.stock-chart{height:245px}}
</style>
