...
<template>
  <div class="quant-page" v-if="!loading">
    ...
    <nav class="q-tabs"><button v-for="t in [{id:'candidates',name:'市场与候选'},{id:'detail',name:'个股分析'},{id:'portfolio',name:'我的持仓'},{id:'validation',name:'模型与数据'}]" :key="t.id" :class="{active:tab===t.id}" @click="tab=t.id">{{ t.name }}</button></nav>
