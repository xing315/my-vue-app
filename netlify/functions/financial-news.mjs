...
const tag=(items,instruments=[])=>{const safe=instruments.filter(i=>/^[036]\d{5}$/.test(i.symbol)&&String(i.name||'').trim().length>=3).slice(0,200);return items.map(item=>{const text=`${item.title} ${item.summary}`;const matches=safe.filter(i=>text.includes(i.symbol)||text.includes(String(i.name).trim()));return{...item,relatedSymbols:matches.map(i=>i.symbol),relevanceScore:matches.length?100:0,matchedBy:matches.length?'股票代码或公司全称':null}})}

export const handler=async event=>{
 try{
  let input={};try{input=JSON.parse(event?.body||'{}')}catch{}const instruments=Array.isArray(input.instruments)?input.instruments:[]
  ...
  return result(200,{mode:'live',fetchedAt:new Date().toISOString(),source:'东方财富财经快讯',items:tag(items,instruments)})
  ...
  try{
   ...
   return result(200,{mode:'live',fetchedAt:new Date().toISOString(),source:'新浪财经7×24',items:tag(items,instruments)})
